...
[System.Serializable] // Đảm bảo lớp Monsters được hiển thị trong Inspector
public class Monsters
{
    ...
    public Monsters(MonsterSaveData saveData)
    {
    ...
    public DamageDatails TakeDamage(Move move, Monsters attacker)
    {
        float critical = 1f;
        ...
        var damageDetails = new DamageDatails()
        {
            ...
            float attack = (move.Base.Category == MoveCategory.Special) ? attacker.GetStat(Stat.SpAttack) : attacker.GetStat(Stat.Attack);
            float defense = (move.Base.Category == MoveCategory.Special) ? GetStat(Stat.SpDefense) : GetStat(Stat.Defense);
            ...
            // Kiểm tra xem quái vật có bị đánh bại không
            if (HP <= 0)
            {
    ...
    public bool OnBeforeMove()
    {
            ...
            {
                Debug.Log($"Calling OnBeforeMove for {Base.Name}");
